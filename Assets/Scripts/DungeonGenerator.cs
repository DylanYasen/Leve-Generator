...
public class LHallWay : HallWay
{
    ...
    public LHallWay(Vector2 start, Vector2 turn, Vector2 end, Room room0, Room room1) : base(start, end, room0, room1)
    {
        ...
        if (startPos.x == turnPos.x)
            cellCount1 = (int)(Mathf.Abs(turnPos.y - startPos.y));

        // horizontal
        else
            cellCount1 = (int)(Mathf.Abs(turnPos.x - startPos.x));

        // size 2 
        // vertical
        if (turnPos.x == endPos.x)
            cellCount2 = (int)(Mathf.Abs(endPos.y - turnPos.y));

        // horizontal
        else
            cellCount2 = (int)(Mathf.Abs(endPos.x - turnPos.x));


        cells1 = new Cell[cellCount1];
        cells2 = new Cell[cellCount2];
    }
